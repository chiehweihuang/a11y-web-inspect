...
Plugin facts：`beacon`、version `3.6.0`、MIT、repository `chiehweihuang/beacon`。
...
每个分数都会附带 `coverage_percent`（实际测量到的 scoring weight 占比）。没有机器证据的类别会以状态（`not-machine-checkable` / `not-applicable`）取代数字。只要有任何证据的类别就会计分；机器检查只有 1-2 项的类别会额外标上 `thin: true`，并在分数旁以同一行呈现"证据薄弱"标示，而不是被隐藏。一旦确认存在癫痫发作风险的 finding（WCAG 2.3.1），无论各类别权重如何，overall score 都会被限制在 0-49 区间。
...
检测器的精度是在没有人为挑选的页面上实测出来的，不是假设出来的。基于对真实抓取站点的调查，针对六个使用量最高的检测器跨站抽样，逐条实例对照其引用的 markup 判定，并进行了对抗性复判：`image-alt` 1.000、`link-name` 0.933、`heading-order` 0.867、`clickable` 0.615、`button-name` 0.600、`input-label` 0.417（各 n=15——置信区间与每条实例判定均随数据一并发布在 [benchmark/2026-08-03-wild-precision/](benchmark/2026-08-03-wild-precision/)）。误报（false positive）的主要成因是由 stylesheet class（而非 inline style）隐藏的 markup，从不加载 CSS 的检测层无法看到这类隐藏；这一局限现已被实测，而不仅仅是被披露。

业界普遍估计自动化工具约涵盖 WCAG 准则的 30-40%。Beacon 实测自己：WCAG 2.2 A+AA 的 55 条准则中，14 条有覆盖（25.5%）、2 条在自动化可及范围内完整决定（3.6%）——逐条对照表与重算方式见 [VALIDATION.md](VALIDATION.md#wcag-criterion-coverage)。
...
Beacon 会把 finding 对照到 23 个追踪法域（`core/scripts/jurisdictions.mjs`，一手来源研究）中相关的 WCAG 条件——14 个法域有专门的网页无障碍法律（US、EU、Japan、Taiwan、Canada、China、South Korea、Brazil、Argentina、Colombia、Peru、Chile、Uruguay、Ecuador），2 个法域只有一般性反歧视框架、没有网页专属条文（Australia、Hong Kong），另外 7 个法域诚实地记录为查无专门法规（Macau、Mongolia、Venezuela、Bolivia、Paraguay、Guyana、Suriname）。这不是法律意见，也不是逐法域机械风险分数；若要宣称合规，仍需确认当地当前有效的要求。
...
Codex adapter 位于 `adapters/codex/`，以 Codex 原生插件方式安装：
